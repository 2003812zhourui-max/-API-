...
MAIN_SHEET = "出库单"
FILTERED_SHEET = "复核时间窗口"
WAVE_SUMMARY_SHEET = "复核波次号去重统计"
PERSON_SUMMARY_SHEET = "复核人员汇总"
...
def review_time_window(kpi_day: date) -> tuple[datetime, datetime]:
...
def is_review_kpi_row(row: tuple[Any, ...], qty_index: int, review_index: int, start_at: datetime, end_at: datetime) -> bool:
    qty = parse_number(row[qty_index])
    review_time = parse_datetime(row[review_index])
    return qty == 1 and review_time is not None and start_at <= review_time <= end_at
...
def build_filtered_sheet(workbook: Any, kpi_day: date) -> tuple[int, int]:
    ...
    start_at, end_at = review_time_window(kpi_day)
    ...
    for row in source.iter_rows(min_row=2, values_only=True):
        if is_review_kpi_row(row, qty_index, review_index, start_at, end_at):
...
def collect_wave_groups(workbook: Any) -> OrderedDict[str, dict[str, Any]]:
    ...
    for row in filtered.iter_rows(min_row=2, values_only=True):
        ...
        if review_time is not None:
            group["min_time"] = review_time if group["min_time"] is None else min(group["min_time"], review_time)
            group["max_time"] = review_time if group["max_time"] is None else max(group["max_time"], review_time)
...
def looks_like_review_event(event: dict[str, Any]) -> bool:
    biz_type = str(event.get("bizType") or "").lower()
    biz_desc = str(event.get("bizDesc") or "")
    return "复核" in biz_desc or "review" in biz_type or "check" in biz_type


def fetch_reviewer_name_from_order(session: requests.Session, record: dict[str, Any]) -> str:
...
def resolve_reviewer_names(
    ...
    for index, (wave_no, group) in enumerate(target_items, 1):
        try:
            ...
            matched_records = [
                record for record in records if str(record.get("deliveryNo") or "").strip() in target_order_nos
            ]
            sample_records = matched_records or records

            names: list[str] = []
            for record in sample_records[:sample_limit]:
                name = fetch_reviewer_name_from_order(session, record)
                if name:
                    names.append(name)
            if names:
                result[wave_no] = Counter(names).most_common(1)[0][0]
            print(f"reviewer {index}/{len(target_items)} {wave_no}: {result.get(wave_no, '') or '-'}", flush=True)
        except Exception as exc:
            result[wave_no] = ""
            print(f"reviewer {index}/{len(target_items)} {wave_no}: failed: {exc}", flush=True)
...
def report_dimensions(kpi_day: date) -> tuple[str, str, str]:
    ...
    return (
        ...
        kpi_day.replace(day=1).strftime("%Y/%m/%d"),
    )
    ...
    for wave_no, group in groups.items():
        summary.append(
            [
                ...
                report_week,
                report_month,
                reviewer_names.get(wave_no) or fallback_name,
                wave_no,
                numeric_qty(group),
                group["count"],
...
def build_person_summary_sheet(workbook: Any, kpi_day: date) -> int:
    ...
    min_index = find_header(headers, "最早复核时间")
    max_index = find_header(headers, "最晚复核时间")
...
def default_output_path(input_path: Path, kpi_day: date) -> Path:
    ...
    return output_dir / f"{input_path.stem}_review_kpi_{kpi_day:%Y%m%d}.xlsx"


def clean_review_workbook(input_path: Path, output_path: Path, kpi_day: date, args: argparse.Namespace) -> None:
    ...
    if args.fetch_reviewer_names:
        ...
        reviewer_names = resolve_reviewer_names(
            ...
            end_time=end_time,
            page_size=args.reviewer_page_size,
            sample_limit=args.reviewer_sample_limit,
            wave_limit=args.reviewer_wave_limit,
            sleep_seconds=args.reviewer_sleep,
        )

    wave_count = build_wave_summary_sheet(workbook, kpi_day, args.name, reviewer_names)
    person_count = build_person_summary_sheet(workbook, kpi_day)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    workbook.save(output_path)

    review_start, review_end = review_time_window(kpi_day)
    create_start, create_end = create_time_window(kpi_day)
    print(f"KPI date: {kpi_day:%Y-%m-%d}")
    print(f"create window: {create_start:%Y-%m-%d %H:%M:%S} ~ {create_end:%Y-%m-%d %H:%M:%S}")
    print(f"review window: {review_start:%Y-%m-%d %H:%M:%S} ~ {review_end:%Y-%m-%d %H:%M:%S}")
    print("filter: Total Qty of SKU = 1, Review time not blank, Review time within window")
    print(f"kept rows: {kept_rows}")
    print(f"removed rows: {removed_rows}")
    print(f"review wave rows: {wave_count}")
    print(f"review person rows: {person_count}")
    if args.fetch_reviewer_names:
        print(f"reviewer names found: {sum(1 for value in reviewer_names.values() if value)}")
    print(f"output file: {output_path.resolve()}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build review KPI workbook from OMP delivery export Excel.")
    parser.add_argument("input", help="Raw or cleaned delivery Excel file path")
    parser.add_argument("--kpi-date", required=True, help="KPI date, YYYY-MM-DD, for example 2026-06-08")
    parser.add_argument("--name", default="", help="Fallback name when reviewer name cannot be fetched")
    parser.add_argument("--output", default="", help="Output Excel path")
    parser.add_argument("--fetch-reviewer-names", action="store_true", help="Fetch reviewer names from OMP progress logs")
    parser.add_argument("--storage-state", default="", help="Optional Playwright storage_state JSON")
    parser.add_argument("--order-start-time", default="", help="Override delivery order search start time")
    parser.add_argument("--order-end-time", default="", help="Override delivery order search end time")
    parser.add_argument("--reviewer-page-size", type=int, default=100)
    parser.add_argument("--reviewer-sample-limit", type=int, default=3)
    parser.add_argument("--reviewer-wave-limit", type=int, default=0, help="Only fetch the first N waves; 0 means all")
    ...
    clean_review_workbook(input_path, output_path, parse_kpi_date(args.kpi_date), args)
